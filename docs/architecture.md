...
Credentials are opaque bearer tokens, stored only as a SHA-256 hash. A request resolves to a `Principal` from **either** an `Authorization: Bearer` header (API/CLI) **or** a browser session cookie. For the web UI the token never lives in the browser: a **Backend-for-Frontend** login exchanges it once for a server-side session and sets an `HttpOnly; Secure; SameSite=Strict` cookie (plus a readable CSRF cookie); unsafe cookie-authenticated writes require a matching `X-CSRF-Token` header (double-submit), and sessions carry sliding-idle + absolute-lifetime timeouts (NIST SP 800-63B). Account management (create / role / activation / token rotation) is admin-only with a last-active-admin guard. Rationale and the government-accessibility framing (USWDS / Section 508, M-22-09) are in [ui-design.md](ui-design.md) §11. Long-term this becomes agency SSO / login.gov (OIDC + PIV/CAC) behind the same session boundary.
